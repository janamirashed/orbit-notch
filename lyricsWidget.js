...
export const OrbitLyricsWidget = GObject.registerClass(
    class OrbitLyricsWidget extends St.DrawingArea {
        ...
        _init(seekCallback = null) {
            ...
            this._hOffset = 0;
            this._hTarget = 0;
            this._marqueeTickId = null;

            this._prevLyrLine = null;
            this._prevAlpha   = 0;
            this._prevDY      = 0;
            this._curAlpha    = 1;
            this._curDY       = 0;
            ...
            this._shimmerPos      = -60;
            this._shimmerGoing    = false;
            this._shimmerCooldown = 150;
            this._pillWidth       = 400;
        ...
        setPaused(paused, title = '') {
            this._paused = paused;
            this._pauseTitle = title;
            this.queue_repaint();
        }

        setHorizontal(v) {
            this._horizontal = v;
            if (v) this._startMarquee(); else this._stopMarquee();
            this.queue_repaint();
        }

        _startMarquee() {
            if (this._marqueeTickId) return;
            this._marqueeTickId = GLib.timeout_add(GLib.PRIORITY_DEFAULT, 16, () => {
                if (!this._horizontal) { this._marqueeTickId = null; return GLib.SOURCE_REMOVE; }
                if (this._state !== 'lyrics') return GLib.SOURCE_CONTINUE;
                let dirty = false;

                const sd = this._hTarget - this._hOffset;
                if (Math.abs(sd) > 0.3) { this._hOffset += sd * 0.12; dirty = true; }
                else if (sd !== 0)       { this._hOffset = this._hTarget; dirty = true; }

                if (this._prevLyrLine && this._prevAlpha > 0.005) {
                    this._prevAlpha += (0 - this._prevAlpha) * 0.20;
                    this._prevDY    += (-9 - this._prevDY)   * 0.20;
                    if (this._prevAlpha <= 0.005) { this._prevLyrLine = null; this._prevAlpha = 0; }
                    dirty = true;
                }

                if (this._curAlpha < 0.999 || Math.abs(this._curDY) > 0.2) {
                    this._curAlpha = Math.min(1, this._curAlpha + (1 - this._curAlpha) * 0.18);
                    this._curDY   += (0 - this._curDY) * 0.18;
                    if (Math.abs(this._curDY) < 0.2) this._curDY = 0;
                    dirty = true;
                }

                if (this._shimmerGoing) {
                    this._shimmerPos += 3.5;
                    if (this._shimmerPos > this._pillWidth + 60) {
                        this._shimmerGoing    = false;
                        this._shimmerCooldown = 220;
                    }
                    dirty = true;
                } else if (this._shimmerCooldown > 0) {
                    if (--this._shimmerCooldown === 0) {
                        this._shimmerPos   = -60;
                        this._shimmerGoing = true;
                    }
                }

                if (dirty) this.queue_repaint();
                return GLib.SOURCE_CONTINUE;
            });
        }

        _stopMarquee() {
            if (this._marqueeTickId) { GLib.Source.remove(this._marqueeTickId); this._marqueeTickId = null; }
        }

        _renderPaused(cr, width, height) {
            const R = this._fgR, G = this._fgG, B = this._fgB;

            const layout = PangoCairo.create_layout(cr);
            layout.set_font_description(Pango.FontDescription.from_string(`Sans Bold ${this._SZ}`));
            layout.set_single_paragraph_mode(true);
            layout.set_text(this._pauseTitle, -1);
            ...
            cr.moveTo(Math.round(Math.max(0, (width - tW) / 2)), Math.round((height - tH) / 2));
            PangoCairo.show_layout(cr, layout);
            cr.restore();
        }

        _renderHorizontal(cr, width, height) {
            if (this._paused) { this._renderPaused(cr, width, height); return; }
            if (this._lyrics.length === 0) return;

            this._pillWidth = width;
            const R = this._fgR, G = this._fgG, B = this._fgB;

            let idx = this._activeIndex < 0 ? 0 : this._activeIndex;
            if (idx >= this._lyrics.length) idx = this._lyrics.length - 1;
            const line     = this._lyrics[idx];
            const nextLine = this._lyrics[idx + 1];

            const nowUs = GLib.get_monotonic_time();
            const posMs = this._pos + Math.min((nowUs - (this._posTs || nowUs)) / 1000, 300);
            const progress = Math.min(1, Math.max(0,
                (posMs - line.time) /
                Math.max(300, (nextLine ? nextLine.time : line.time + 5000) - line.time)
            ));

            const lyrFontDesc = Pango.FontDescription.from_string(`Sans Bold ${this._SZ}`);
            const lyrLayout   = PangoCairo.create_layout(cr);
            lyrLayout.set_font_description(lyrFontDesc);
            lyrLayout.set_single_paragraph_mode(true);
            lyrLayout.set_text(line.text, -1);
            const [lyrW, lyrH] = lyrLayout.get_pixel_size();

            const lyrBaseY = Math.round((height - lyrH) / 2);

            this._hTarget = lyrW > width
                ? Math.max(0, Math.min(lyrW - width, lyrW * progress - width * 0.38))
                : 0;
            const startX = lyrW <= width ? (width - lyrW) / 2 : -this._hOffset;
            ...
            if (this._prevLyrLine && this._prevAlpha > 0.005) {
                const prevLayout = PangoCairo.create_layout(cr);
                prevLayout.set_font_description(lyrFontDesc);
                prevLayout.set_single_paragraph_mode(true);
                prevLayout.set_text(this._prevLyrLine, -1);
                const [pw] = prevLayout.get_pixel_size();
                const prevX = pw <= width ? (width - pw) / 2 : 0;
                cr.setSourceRGBA(R, G, B, this._prevAlpha);
                cr.moveTo(prevX, lyrBaseY + this._prevDY);
                PangoCairo.show_layout(cr, prevLayout);
            }

            const curY = lyrBaseY + this._curDY;
            cr.setSourceRGBA(R, G, B, this._curAlpha);
            cr.moveTo(startX, curY);
            PangoCairo.show_layout(cr, lyrLayout);

            if (this._shimmerGoing && this._curAlpha >= 0.99 && this._curDY === 0) {
                const SW    = 75;
                const sx    = this._shimmerPos;
                const clipL = Math.max(0, sx - SW);
                const clipW = Math.min(width, sx + SW) - clipL;
                if (clipW > 0) {
                    const grad = new Cairo.LinearGradient(sx - SW, 0, sx + SW, 0);
                    grad.addColorStopRGBA(0,    R, G, B, 0.0);
                    grad.addColorStopRGBA(0.35, R, G, B, 0.22);
                    grad.addColorStopRGBA(0.5,  R, G, B, 0.22);
                    grad.addColorStopRGBA(0.65, R, G, B, 0.22);
                    grad.addColorStopRGBA(1,    R, G, B, 0.0);
                    cr.save();
                    cr.rectangle(clipL, curY - 1, clipW, lyrH + 2);
                    cr.clip();
                    cr.setSource(grad);
                    cr.moveTo(startX, curY);
                    PangoCairo.show_layout(cr, lyrLayout);
        ...
        setLyrics(lyrics) {
            ...
            this._geomBuilt = false;
            if (this._horizontal) this._startMarquee();
            this.queue_repaint();
        }
        ...
        updatePosition(timeMs) {
            ...
            this._pos   = timeMs;
            this._posTs = GLib.get_monotonic_time();
            ...
            if (this._horizontal) {
                ...
                const oldText = (this._activeIndex >= 0 && this._activeIndex < this._lyrics.length)
                    ? this._lyrics[this._activeIndex].text : null;

                this._prevLyrLine = oldText;
                this._prevAlpha   = oldText ? Math.min(this._curAlpha, 1) : 0;
                this._prevDY      = this._curDY;
                this._curAlpha    = 0;
                this._curDY       = 8;

                this._hOffset = 0; this._hTarget = 0;
                this._shimmerGoing    = false;
                this._shimmerCooldown = 22;
                this._shimmerPos      = -60;
            }
        ...
        _onRepaint() {
            ...
            if (this._horizontal) {
                this._renderHorizontal(cr, width, height);
                cr.$dispose();
                return;
            }
