...
export class LyricsClient {
  ...
  async getLyrics(title, artist, album, duration, settings) {
    ...
    try {
      ...
      console.debug(`orbit: Lyrics fetch error: ${e.message}`);
      throw e;
    }
  }
  ...
  async _fetchExact(url) {
    ...
    try {
      ...
      console.debug(`orbit: Exact lyrics fetch error: ${e.message}`);
      return null;
    }
  }
  ...
  async _fetchCandidates(title, artist, duration) {
    ...
    try {
      ...
      console.debug(`orbit: Lyrics search error: ${e.message}`);
      throw e;
    }
  }
